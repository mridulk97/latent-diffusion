...
if __name__ == "__main__":
    ...
    for _, val in class_to_node_dict.items():
        num_levels = len(val[0].split(','))
        break

    ancestral_level = []
    for i in range(num_levels-1):
        ancestral_level.append(reorganize_dict_based_on_index(class_to_node_dict, index=i))
    ...
    ancestral_level_final = []
    for ancestor in ancestral_level:
        ancestral_level_final.append(get_ancester_hier_embed_list(ancestor))
    ...
    for i in range(num_levels-1):
        ...
        sampling(model, sampler, ancestral_level_final[ancestral_lvl-1], opt.n_samples, masked=True, levels=i+1, ancestry_level=ancestral_lvl, num_levels=num_levels)
